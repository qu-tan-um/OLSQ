...
T_input = 0
if len(sys.argv) > 4:
    T_input = literal_eval(sys.argv[4])

new_file_name = "result/paper/" + file_name.split('/')[-1].replace('.qasm', "_{}_olsq_{}".format(device_name, objective_name))


device_test = qcdevice(device_name)
N = device_test.count_physical_qubit
e = device_test.connection_list
S = device_test.swap_duration
...
not_solve = True
while(not_solve):
    print("Trying maximal depth = {}...".format(T))
...
# for depth optimization
    ...
    z3.add(time[12] <= time[13])
    z3.add(time[13] <= time[18])
    z3.add(time[18] <= time[19])
    z3.add(time[19] <= time[21])
    """
    ...
    for t in range(S - 1, T):
        for k in range(K):
            for tt in range(t - S + 1, t):
                z3.add(Implies(sigma[k][t] == True, sigma[k][tt] == False))
    ...
    for t in range(S - 1, T):
        for k in range(K):
            for tt in range(t - S + 1, t + 1):
                for l in range(L):
                    if l in G_1:
                        z3.add(Implies(And(time[l] == tt, Or(space[l] == e[k][0], space[l] == e[k][1])), sigma[k][t] == False))
                    elif l in G_2:
                        z3.add(Implies(And(time[l] == tt, space[l] == k), sigma[k][t] == False))
                        for kk in O[k]:
                            z3.add(Implies(And(time[l] == tt, space[l] == kk), sigma[k][t] == False))
...
if result_depth > model[time[l]].as_long() + 1:
    result_depth = model[time[l]].as_long() + 1
for m in range(M):
...
for k in range(K):
    for t in range(result_depth):
        if model[sigma[k][t]]:
            ...
            scheduled[t - 2].append("cx q[{}], q[{}]; // SWAP on edge {} at time {}\n".format(e[k][0], e[k][1], k, t))
            scheduled[t - 1].append("cx q[{}], q[{}]; // SWAP on edge {} at time {}\n".format(e[k][1], e[k][0], k, t))
            scheduled[t].append("cx q[{}], q[{}]; // SWAP on edge {} at time {}\n".format(e[k][0], e[k][1], k, t))
...
# outFile.writelines(['\n', "measure q -> c;\n"])
for m in range(M):
    outFile.write("measure q[{}] -> c[{}];\n".format(model[pi[m][result_depth - 1]].as_long(), m))
outFile.close()
# for t in range(T):
#     for m in range(M):
#         print("logical qubit {} is mapped to node {} at cycle {}".format(m, model[pi[m][t]], t))
